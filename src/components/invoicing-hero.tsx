...
import { InvoicingMockup } from "@/components/invoicing-mockup"
import { Button } from "@/components/ui/button"
import { ChevronRight } from "lucide-react"

export function InvoicingHero({ isLoggedIn = false }: { isLoggedIn?: boolean }) {
  return (
        ...
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto mb-20">
          ...
          >
            <span className="inline-block px-3 py-1 text-sm font-medium text-primary bg-primary/10 rounded-full mb-6 italic">
              Enterprise-grade Billing & Invoicing
            </span>
          </motion.div>
          ...
          >
            Automated Invoicing for <br />
            <span className="text-primary italic">Global</span> Scale.
          </motion.h1>
          ...
          >
            Create, manage, and send professional invoices that support both crypto and fiat.
            Automate recurring billing and streamline your reconciliation process.
          </motion.p>
          ...
          >
            <Button asChild size="lg" className="h-14 px-10 text-lg font-semibold rounded-full shadow-lg hover:shadow-primary/20 transition-all">
              <a href={isLoggedIn ? "https://dashboard.payerone.com/billing?tab=manage" : "https://account.dhru.com/register?for=payerone.com"}>
                {isLoggedIn ? "Manage Billing" : "Start Invoicing"}
