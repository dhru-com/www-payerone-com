...
import { SubscriptionMockup } from "@/components/subscription-mockup"
import { Button } from "@/components/ui/button"
import { ChevronRight } from "lucide-react"

export function SubscriptionHero({ isLoggedIn = false }: { isLoggedIn?: boolean }) {
  return (
        ...
        <div className="flex flex-col items-center text-center max-w-4xl mx-auto mb-20">
          ...
          >
            <span className="inline-block px-3 py-1 text-sm font-medium text-primary bg-primary/10 rounded-full mb-6 italic">
              Automated Web3 Recurring Billing
            </span>
          </motion.div>
          ...
          >
            The engine for your recurring revenue. Handle trials, upgrades, and
            automated collections across crypto and fiat with enterprise-grade reliability.
          </motion.p>
          ...
          >
            <Button asChild size="lg" className="h-14 px-10 text-lg font-semibold rounded-full shadow-lg hover:shadow-primary/20 transition-all">
              <a href={isLoggedIn ? "https://dashboard.payerone.com/billing?tab=manage" : "https://account.dhru.com/register?for=payerone.com"}>
                {isLoggedIn ? "Manage Billing" : "Start Scaling"}
