  ...
  return (
    <section className="py-24 bg-background relative overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {networks.map((network, index) => (
            <motion.div
              ...
              className="group p-5 md:p-6 rounded-2xl border bg-card hover:bg-secondary/30 transition-all duration-300"
            >
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-muted dark:bg-zinc-800 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-white dark:group-hover:bg-zinc-700 transition-all duration-500 shadow-sm">
                <Image
                  src={network.logo}
                  alt={`${network.name} logo`}
                  width={32}
                  height={32}
                  className="w-6 h-6 md:w-8 md:h-8 object-contain dark:invert dark:brightness-0 group-hover:dark:invert-0 group-hover:dark:brightness-100 transition-all"
                />
              </div>
              <div className="flex items-center gap-2 mb-2">
                <h3 className="font-bold text-foreground text-xs md:text-sm">{network.name}</h3>
        ...
        <div className="mt-16 flex flex-col md:flex-row items-center justify-center gap-6 md:gap-8 p-6 md:p-8 rounded-2xl bg-muted/30 border">
          <div className="text-center md:text-left">
            <h4 className="text-base md:text-lg font-bold mb-1">High-Liquidity Assets</h4>
            <p className="text-xs md:text-sm text-muted-foreground">Native settlement for the world&apos;s most trusted stablecoins.</p>
          </div>
          <div className="flex flex-wrap justify-center gap-4 md:gap-6">
            {assets.map((asset) => (
              <div key={asset.name} className="flex items-center gap-3 px-4 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl bg-white dark:bg-zinc-800 border dark:border-zinc-700 shadow-sm group hover:scale-105 transition-transform cursor-pointer">
                ...
                <div>
                  <div className="text-xs md:text-sm font-bold leading-none">{asset.name}</div>
                  <div className="text-[9px] md:text-[10px] text-muted-foreground font-medium uppercase tracking-wider">{asset.fullName}</div>
                </div>
              </div>
            ))}
          </div>
