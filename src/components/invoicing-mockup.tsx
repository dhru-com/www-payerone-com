...
import { FileText, Download, Send, CreditCard, Wallet, CheckCircle2, MoreVertical, Plus } from "lucide-react"
...
export function InvoicingMockup() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto rounded-[2.5rem] border bg-zinc-50/50 p-4 md:p-8 shadow-2xl overflow-hidden text-foreground">
      <div className="bg-white rounded-[2rem] border shadow-sm overflow-hidden flex flex-col lg:flex-row min-h-[600px]">
        {/* Sidebar / Actions */}
        <div className="w-full lg:w-72 bg-zinc-50 border-r p-8 space-y-8">
          <div>
            <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-4">Invoice #INV-2026-001</div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold uppercase tracking-wider">
          ...
          <div className="space-y-3">
            <button className="w-full flex items-center justify-between p-4 rounded-2xl bg-primary text-primary-foreground font-bold text-sm shadow-lg hover:scale-[1.02] transition-transform">
              <div className="flex items-center gap-3">
                <Send className="h-4 w-4" />
                Send Invoice
              </div>
            </button>
            <button className="w-full flex items-center justify-between p-4 rounded-2xl bg-white border font-bold text-sm hover:bg-zinc-50 transition-colors">
              <div className="flex items-center gap-3">
        ...
        <div className="flex-grow p-8 md:p-12 space-y-12">
          {/* Header */}
          <div className="flex justify-between items-start">
            <div className="space-y-4">
               <div className="w-12 h-12 rounded-xl bg-black flex items-center justify-center text-white shadow-lg overflow-hidden">
                  <span className="font-black text-lg">P1</span>
               </div>
               <div>
                  <h3 className="text-xl font-bold">PayerOne Inc.</h3>
                  <p className="text-sm text-zinc-500">221B Baker Street<br />London, NW1 6XE</p>
               </div>
            </div>
            <div className="text-right space-y-1">
               <h2 className="text-3xl font-bold tracking-tighter uppercase">Invoice</h2>
               <p className="text-sm text-zinc-500 font-medium">Billed to: Global Tech Corp</p>
            </div>
          </div>
             ...
             <table className="w-full text-left">
                ...
                <tbody className="text-sm border-t">
                   {lineItems.map((item, i) => (
                      <tr key={i} className="border-b group hover:bg-zinc-50 transition-colors">
                         <td className="py-5 font-medium">{item.desc}</td>
                         <td className="py-5 text-center text-zinc-500">{item.qty}</td>
                         <td className="py-5 text-right text-zinc-500">${item.price.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
      ...
      <motion.div
        ...
        className="mt-8 bg-zinc-900 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-6"
      >
        <div className="flex items-center gap-6">
           <div className="p-3 rounded-2xl bg-white/10 flex items-center justify-center">
        ...
        <div className="flex gap-4 w-full md:w-auto">
           <button className="flex-grow md:flex-none px-8 h-12 rounded-2xl bg-white text-black font-bold text-sm hover:bg-zinc-100 transition-colors">
              Pay Now
           </button>
           <button className="p-3 rounded-2xl border border-white/20 text-white hover:bg-white/5 transition-colors">
